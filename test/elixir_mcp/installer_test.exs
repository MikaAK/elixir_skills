defmodule ElixirMcp.InstallerTest do
  ...
  setup do
    ...
    Application.put_env(:elixir_mcp, :skills_target_dir, tmp_dir)

    on_exit(fn ->
      File.rm_rf!(tmp_dir)
      Application.delete_env(:elixir_mcp, :skills_target_dir)
    end)
  ...
  defp make_skill(id, package \\ :fake_dep) do
    source = Path.join([@fixtures_path, "fake_dep", "priv", "claude_skills", "test-skill"])
  ...
  describe "plan/1" do
    test "marks new skills as :new", %{tmp_dir: _tmp_dir} do
      skill = make_skill("my-skill")
      [entry] = Installer.plan([skill])
      assert entry.action === :new
    end

    test "marks installed same-version skills as :unchanged", %{tmp_dir: tmp_dir} do
      skill = make_skill("my-skill")
    ...
    test "marks conflicting unmanaged skills as :conflict", %{tmp_dir: tmp_dir} do
      skill = make_skill("my-skill")
      target = Path.join(tmp_dir, skill.namespaced_id)
      File.mkdir_p!(target)
  ...
  describe "execute/2" do
    test "creates symlinks for new skills", %{tmp_dir: tmp_dir} do
      skill = make_skill("my-skill")
      plan = [%{skill: skill, action: :new, reason: nil}]

      assert {:ok, %{installed: [_], skipped: []}} = Installer.execute(plan)
    ...
    test "copies when :copy option is set", %{tmp_dir: tmp_dir} do
      skill = make_skill("copy-skill")
      plan = [%{skill: skill, action: :new, reason: nil}]

      assert {:ok, %{installed: [_], skipped: []}} = Installer.execute(plan, copy: true)
    ...
    test "skips conflicts without force", %{tmp_dir: _tmp_dir} do
      skill = make_skill("conflict-skill")
      plan = [%{skill: skill, action: :conflict, reason: "exists"}]

      assert {:ok, %{installed: [], skipped: [_]}} = Installer.execute(plan)
    end

    test "writes tracking file", %{tmp_dir: tmp_dir} do
  ...
  describe "uninstall/1" do
    test "removes installed skills", %{tmp_dir: tmp_dir} do
      ...
      {:ok, removed} = Installer.uninstall([skill.namespaced_id])
      assert skill.namespaced_id in removed
      refute File.exists?(target)
    end

    test "uninstall :all removes everything", %{tmp_dir: _tmp_dir} do
      ...
      assert Enum.empty?(Installer.read_tracking())
    end
  end
